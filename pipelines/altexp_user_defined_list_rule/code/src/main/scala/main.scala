...
object Main {
  ...
  def apply(context: Context): Unit = {
    val df_Source_Table12    = Source_Table12(context)
    val df_Source_Table_sync = Source_Table_sync(context, df_Source_Table12)
    ...
    OFILE12(context,       df_Remove_Duplicate_on_Key_ss)
    REJECT_FILE12(context, df_Filter_Condition_out1)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri",
                   "pipelines/altexp_user_defined_list_rule"
    )
    registerUDFs(spark)
