  ...
  def apply(context: Context, in: DataFrame): DataFrame = {
    ...
    def frmlry_url() = {
      when(
        ...
        ) =!= lit("FORMULARY_NOT_FOUND")).and(col("future_flg") === lit(1)),
        when(
          lookup(
            "Formulary_Override_Ref",
            col("formulary_id"),
          ...
          ).getField("run_eff_dt") > col("as_of_dt"),
    ...
    def cag_url() = {
      when(
        ...
        coalesce(
          lookup("CAG_Override_Ref", col("carrier"), col("account"), col("group"), col("future_flg")).getField("data_path"),
          lookup("CAG_Override_Ref", col("carrier"), col("account"), lit("*ALL"),  col("future_flg")).getField("data_path"),
          lookup("CAG_Override_Ref", col("carrier"), lit("*ALL"),    lit("*ALL"),  col("future_flg")).getField("data_path"),
          lookup("CAG_Override_Ref", lit("*ALL"),    lit("*ALL"),    lit("*ALL"),  col("future_flg")).getField("data_path"),
          lookup("CAG_Override_Ref", lit(null),      lit(null),      lit(null),    col("future_flg")).getField("data_path"),
          lit("CAG_NOT_FOUND")
    ...
    def filter_output_profile_record(
      ...
      var is_current_form: org.apache.spark.sql.Column =
        when(is_future_snap_form.cast(BooleanType), lit("Future")).otherwise(lit("Current"))
      var is_current_cag: org.apache.spark.sql.Column =
        when(is_future_snap_cag.cast(BooleanType), lit("Future")).otherwise(lit("Current"))
          ...
          array_join(
            array(
              when(
                ...
                array_join(array(lit("Invalid TAL="),
                                 col("tal_name"),
              ...
              when(
                ...
                array_join(array(lit("Invalid TAC="),
                                 col("tac_name"),
                                 lit(" used for Output Profile="),
                                 col("output_profile_name"),
                                 lit(".")
              ...
              when(
                ...
                array_join(array(lit("Invalid TAR="),
                                 col("tar_name"),
                                 lit(" used for Output Profile="),
                                 col("output_profile_name"),
                                 lit(".")
              ...
              when(
                ...
                array_join(array(lit("Invalid TSD="),
                                 col("tsd_name"),
                                 lit(" used for Output Profile="),
                                 col("output_profile_name"),
                                 lit(".")
              ...
              when(
                ...
                array_join(array(is_current_cag,
                                 lit(" Dated Drug data snapshot is not available for Output Profile="),
                                 col("output_profile_name"),
                                 lit(".")
                           ),
                           ""
              ...
              when(
                ...
                array_join(
                  array(
                    is_current_form,
                    lit(" Dated Formulary data snapshot is not available for Output Profile="),
                    col("output_profile_name"),
                    lit(" and Formulary="),
                    col("formulary_name"),
                    lit(" and Customer="),
              ...
              when(
                ...
                array_join(
                  array(
                    is_current_form,
                    lit(" Dated Formulary data snapshot is not having valid run_eff_dt for Output Profile="),
                    col("output_profile_name"),
                    lit(" and Formulary="),
                    col("formulary_name"),
                    lit(" and Customer="),
