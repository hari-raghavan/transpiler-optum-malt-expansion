...
object Main {
  ...
  def apply(context: Context): Unit = {
    val df_LKP_Lookup11 = LKP_Lookup11(context)
    if (
      ...
      LKP_Lookup_lookup(context, df_LKP_Lookup11)
    val df_Source_Table11    = Source_Table11(context)
    val df_Source_Table_sync = Source_Table_sync(context, df_Source_Table11)
    ...
    OFILE11(context,       df_Remove_Duplicate_on_Key_ss)
    REJECT_FILE11(context, df_Filter_Condition_out1)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri", "pipelines/altexp_tsd_dtl")
    registerUDFs(spark)
    MetricsCollector.instrument(spark, "pipelines/altexp_tsd_dtl") {
      apply(context)
    }
  }
