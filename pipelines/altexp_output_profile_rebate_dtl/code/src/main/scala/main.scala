...
object Main {
  ...
  def apply(context: Context): Unit = {
    val df_LKP_Lookup3 = LKP_Lookup3(context)
    if (
      ...
      LKP_Lookup_lookup(context, df_LKP_Lookup3)
    val df_Source_Table3     = Source_Table3(context)
    val df_Source_Table_sync = Source_Table_sync(context, df_Source_Table3)
    ...
    OFILE3(context,       df_Remove_Duplicate_on_Key_ss)
    REJECT_FILE3(context, df_Filter_Condition_out1)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri",
                   "pipelines/altexp_output_profile_rebate_dtl"
    )
    registerUDFs(spark)
