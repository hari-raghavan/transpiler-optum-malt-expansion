...
object Main {
  ...
  def apply(context: Context): Unit = {
    ...
    Reference_File_CAG(context, df_Reference_File_Creation)
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf
      .set("prophecy.metadata.pipeline.uri", "pipelines/altexp_cag_ovrrd_xwalk")
    registerUDFs(spark)
    MetricsCollector.instrument(spark, "pipelines/altexp_cag_ovrrd_xwalk") {
      apply(context)
    }
  }
