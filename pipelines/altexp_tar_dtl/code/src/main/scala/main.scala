...
object Main {
  ...
  def apply(context: Context): Unit = {
    val df_LKP_Lookup9  = LKP_Lookup9(context)
    val df_LKP_Lookup19 = LKP_Lookup19(context)
    if (
      ...
      LKP_Lookup_lookup(context, df_LKP_Lookup9)
    if (
      _root_.io.prophecy.abinitio.ScalaFunctions._not(
        _root_.io.prophecy.abinitio.ScalaFunctions.convertToBoolean(
          _root_.io.prophecy.abinitio.ScalaFunctions
      ...
      LKP_Lookup1_lookup(context, df_LKP_Lookup19)
    val df_Source_Table9     = Source_Table9(context)
    val df_Source_Table_sync = Source_Table_sync(context, df_Source_Table9)
    ...
    OFILE9(context,       df_Remove_Duplicate_on_Key_ss)
    REJECT_FILE9(context, df_Filter_Condition_out1)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri", "pipelines/altexp_tar_dtl")
    registerUDFs(spark)
    MetricsCollector.instrument(spark, "pipelines/altexp_tar_dtl") {
      apply(context)
    }
  }
