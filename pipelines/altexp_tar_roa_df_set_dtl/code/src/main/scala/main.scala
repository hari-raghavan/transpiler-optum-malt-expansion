...
object Main {
  ...
  def apply(context: Context): Unit = {
    val df_LKP_Lookup10 = LKP_Lookup10(context)
    if (
      ...
      LKP_Lookup_lookup(context, df_LKP_Lookup10)
    val df_Source_Table10    = Source_Table10(context)
    val df_Source_Table_sync = Source_Table_sync(context, df_Source_Table10)
    ...
    OFILE10(context,       df_Remove_Duplicate_on_Key_ss)
    REJECT_FILE10(context, df_Filter_Condition_out1)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri",
                   "pipelines/altexp_tar_roa_df_set_dtl"
    )
    registerUDFs(spark)
    MetricsCollector.instrument(spark, "pipelines/altexp_tar_roa_df_set_dtl") {
      apply(context)
    }
  }
