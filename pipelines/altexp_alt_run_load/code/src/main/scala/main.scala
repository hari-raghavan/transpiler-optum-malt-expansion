...
object Main {
  ...
  def apply(context: Context): Unit = {
    val df_Join_With_Input_Surrogate_Key_File__Input_Surrogate_Key_File_2 =
      if (
        ...
        Join_With_Input_Surrogate_Key_File__Input_Surrogate_Key_File_2(context)
      else null
      ...
      if (
        ...
        val df_Join_With_Input_Surrogate_Key_File__Join =
          Join_With_Input_Surrogate_Key_File__Join(
            context,
            df_Join_With_Input_Surrogate_Key_File__Input_Surrogate_Key_File_2,
            df_Transform_Logic
          )
    ...
    if (
      ...
      Load_Serial_log_file_2(context, df_Handle_Serial_Logs)
    }
    ...
    if (
      ...
      Append_Mode_2(context, df_Append_Mode_DateTimeNormalize)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf
      .set("prophecy.metadata.pipeline.uri", "pipelines/altexp_alt_run_load")
    registerUDFs(spark)
    MetricsCollector.instrument(spark, "pipelines/altexp_alt_run_load") {
      apply(context)
    }
  }
