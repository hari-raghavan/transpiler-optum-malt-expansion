package io.prophecy.pipelines.altexp_xml_rules_parser3

import io.prophecy.libs._
import io.prophecy.pipelines.altexp_xml_rules_parser3.config._
...
import io.prophecy.pipelines.altexp_xml_rules_parser3.graph._
...
object Main {
  ...
  def apply(context: Context): Unit = {
    ...
    val df_calculate_offset =
      calculate_offset(context, df_Convert_XMLTYPE_into_DML_described_format)
    val df_Group_the_Rules_and_assign_priority =
      Group_the_Rules_and_assign_priority(context, df_calculate_offset)
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri",
                   "pipelines/altexp_xml_rules_parser3"
    )
    registerUDFs(spark)
    MetricsCollector.instrument(spark, "pipelines/altexp_xml_rules_parser3") {
      apply(context)
    }
  }
