...
object Main {
  ...
  def apply(context: Context): Unit = {
    val df_Source_Table2     = Source_Table2(context)
    val df_Source_Table_sync = Source_Table_sync(context, df_Source_Table2)
    ...
    OFILE2(context,       df_Remove_Duplicate_on_Key_ss)
    REJECT_FILE2(context, df_Filter_Condition_out1)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf
      .set("prophecy.metadata.pipeline.uri", "pipelines/altexp_output_profile")
    registerUDFs(spark)
    MetricsCollector.instrument(spark, "pipelines/altexp_output_profile") {
      apply(context)
    }
  }
