...
object Main {
  ...
  def apply(context: Context): Unit = {
    val df_Source_Table6     = Source_Table6(context)
    val df_Source_Table_sync = Source_Table_sync(context, df_Source_Table6)
    ...
    OFILE6(context,       df_Remove_Duplicate_on_Key_ss)
    REJECT_FILE6(context, df_Filter_Condition_out1)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri",
                   "pipelines/altexp_tal_assoc_clinical_indn"
    )
    registerUDFs(spark)
