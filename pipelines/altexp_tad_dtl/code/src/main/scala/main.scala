...
object Main {
  ...
  def apply(context: Context): Unit = {
    val df_Source_Table5     = Source_Table5(context)
    val df_Source_Table_sync = Source_Table_sync(context, df_Source_Table5)
    ...
    OFILE5(context,       df_Remove_Duplicate_on_Key_ss)
    REJECT_FILE5(context, df_Filter_Condition_out1)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri", "pipelines/altexp_tad_dtl")
    registerUDFs(spark)
    MetricsCollector.instrument(spark, "pipelines/altexp_tad_dtl") {
      apply(context)
    }
  }
