...
object Main {
  ...
  def apply(context: Context): Unit = {
    val df_Source_Table8     = Source_Table8(context)
    val df_Source_Table_sync = Source_Table_sync(context, df_Source_Table8)
    ...
    OFILE8(context,       df_Remove_Duplicate_on_Key_ss)
    REJECT_FILE8(context, df_Filter_Condition_out1)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri", "pipelines/altexp_tal_dtl")
    registerUDFs(spark)
    MetricsCollector.instrument(spark, "pipelines/altexp_tal_dtl") {
      apply(context)
    }
  }
