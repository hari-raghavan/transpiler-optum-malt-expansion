...
object Main {
  ...
  def apply(context: Context): Unit = {
    val df_Source_Table_16   = Source_Table_16(context)
    val df_Source_Table_sync = Source_Table_sync(context, df_Source_Table_16)
    ...
    OFILE_16(context,       df_Remove_Duplicate_on_Key_ss)
    REJECT_FILE_16(context, df_Filter_Condition_out1)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri",
                   "pipelines/altexp_gpi_rank_ratio_pset"
    )
    registerUDFs(spark)
    MetricsCollector.instrument(spark, "pipelines/altexp_gpi_rank_ratio_pset") {
      apply(context)
    }
  }
