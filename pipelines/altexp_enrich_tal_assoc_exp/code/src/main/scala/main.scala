...
object Main {
  ...
  def apply(context: Context): Unit = {
    val df_IFILE_Input_File_1 = IFILE_Input_File_1(context)
    val df_Create_Exp_Tab     = Create_Exp_Tab(context, df_IFILE_Input_File_1)
    val df_Create_Criteria_Tab =
      Create_Criteria_Tab(context, df_IFILE_Input_File_1)
    ...
    val df_Products_1 = Products_1(context)
    Products_lookup(context, df_Products_1)
    ...
    Partial_UDL_TALA_TAL_Output_1(context, df_check_records_limitReformat_1)
    Flag_1(context,                        df_Count_of_records_Reformat)
    val df_check_records_limitReformat_0 = check_records_limitReformat_0(
      context,
      df_check_records_limit_RowDistributor_out0
    )
    Write_Excel_Enrich_Product_Data_1(context, df_check_records_limitReformat_0)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri",
                   "pipelines/altexp_enrich_tal_assoc_exp"
    )
    registerUDFs(spark)
