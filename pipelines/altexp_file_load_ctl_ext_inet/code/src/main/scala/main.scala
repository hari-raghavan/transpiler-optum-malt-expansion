...
object Main {
  ...
  def apply(context: Context): Unit = {
    val df_Source_Table_15   = Source_Table_15(context)
    val df_Source_Table_sync = Source_Table_sync(context, df_Source_Table_15)
    ...
    OFILE_15(context,       df_Remove_Duplicate_on_Key_ss)
    REJECT_FILE_15(context, df_Filter_Condition_out1)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri",
                   "pipelines/altexp_file_load_ctl_ext_inet"
    )
    registerUDFs(spark)
