...
package object Join_With_DB {
  ...
  def apply(context: Context, in: DataFrame): DataFrame = {
    val df_Join_with_DB_jdbc1 = Join_with_DB_jdbc1(context)
    val df_Join_with_DB_jdbcdb_predicate_pushdown =
      Join_with_DB_jdbcdb_predicate_pushdown(context, df_Join_with_DB_jdbc1, in)
    ...
    Load_MFS_log_file1(context, df_Join_with_DB_Log)
    df_Join_with_DB
  }
