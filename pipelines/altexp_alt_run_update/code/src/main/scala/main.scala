...
object Main {
  ...
  def apply(context: Context): Unit = {
    val df_Join_With_Input_Surrogate_Key_File__Input_Surrogate_Key_File_3 =
      if (
        ...
        Join_With_Input_Surrogate_Key_File__Input_Surrogate_Key_File_3(context)
      else null
    ...
    val df_Transform_Logic_Filter_select =
      Transform_Logic_Filter_select(context, df_SORT)
    val df_Transform_Logic_Reformat =
      Transform_Logic_Reformat(context, df_Transform_Logic_Filter_select)
      ...
      if (
        ...
        val df_Join_With_Input_Surrogate_Key_File__Join =
          Join_With_Input_Surrogate_Key_File__Join(
            context,
            df_Join_With_Input_Surrogate_Key_File__Input_Surrogate_Key_File_3,
            df_Transform_Logic_Reformat
          )
    ...
    if (
      ...
      Load_Serial_log_file_3(context, df_Handle_Serial_Logs)
    }
    ...
    if (
      ...
      Append_Mode_3(context, df_Append_Mode_DateTimeNormalize)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf
      .set("prophecy.metadata.pipeline.uri", "pipelines/altexp_alt_run_update")
    registerUDFs(spark)
    MetricsCollector.instrument(spark, "pipelines/altexp_alt_run_update") {
      apply(context)
    }
  }
