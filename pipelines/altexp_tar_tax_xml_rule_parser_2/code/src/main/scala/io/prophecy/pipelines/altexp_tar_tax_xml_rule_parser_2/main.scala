package io.prophecy.pipelines.altexp_tar_tax_xml_rule_parser_2

import io.prophecy.libs._
import io.prophecy.pipelines.altexp_tar_tax_xml_rule_parser_2.config._
...
import io.prophecy.pipelines.altexp_tar_tax_xml_rule_parser_2.graph._
...
object Main {
  ...
  def apply(context: Context): Unit = {
    val df_IFILE = IFILE(context)
    val df_Convert_Target_XMLTYPE_into_DML_described_format =
      Convert_Target_XMLTYPE_into_DML_described_format(context, df_IFILE)
    ...
    val df_Convert_Alternative_XMLTYPE_into_DML_described_format =
      Convert_Alternative_XMLTYPE_into_DML_described_format(context, df_IFILE)
    ...
    OFILE_Rule_Xwalk(context, df_reformat_tac_data)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri",
                   "pipelines/altexp_tar_tax_xml_rule_parser_2"
    )
    registerUDFs(spark)
